...
// rsagen命令生成rsa。来自PEM编码的RSA公钥的PublicKey变量。
package main
...
func run() error {
	...
	var (
		inPath       = flag.String("f", ".\\cmd\\rsagen\\input.pem", "input path (defaults to stdin)")
		outPath      = flag.String("o", ".\\cmd\\rsagen\\out.go", "output path (defaults to stdout)")
